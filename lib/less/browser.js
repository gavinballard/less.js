...
less.refresh = function (reload) {
    ...
    loadStyleSheets(function (root, sheet, env) {
        if (env.local) {
            ...
            // @TODO: Don't rebuild CSS here if we don't have to.
            var preParsed = getParsedSheet(sheet.href);
            createCSS(preParsed.toCSS(), sheet, env.lastModified);

        } else {
            log("parsed " + sheet.href + " successfully.");

            // Create a reference hash of all variables in the parsed tree.
            root._variableHash = getVariableHash(root);

            // Store the parsed object.
            setParsedSheet(sheet.href, root);

            // Generate CSS from the parsed object.
            createCSS(root.toCSS(), sheet, env.lastModified);
...
less.updateVariable = function(variable, value, sheetHref) {
    ...
    // Construct the expression and attempt to parse it.
    var expression = variable + ': ' + value + ';';
    try {
        new(less.Parser)().parse(expression, function(e, tree) {
            if(e !== null) {
                error(e, '');
            } else {
...
function updatedVariableParsed(variable, parsed, sheetHref) {
    ...
    for(var i = 0; i < less.sheets.length; i++) {
        ...
        if(typeof(sheet) === 'undefined' || href == sheetHref) {
            ...
            var replacements = getAllVariables(parsed);

            // Iterate through each replacement.
            replacements.forEach(function(replacement) {
                if(parsedSheet._variableHash.hasOwnProperty(replacement.name)) {
                    var existing = parsedSheet._variableHash[replacement.name];
                    existing.value = replacement.value;
                }                
            }.bind(this));

            // Re-store the parsed sheet.
            setParsedSheet(href, parsedSheet);
        }
    }
}
...
function getVariableHash(node) {
    var hash = {};
    getAllVariables(node).forEach(function(variable) {
       hash[variable.name] = variable;
    });
    return hash;
}

function getAllVariables(node) {

    // If it's a rule, check for variable.
    if(node instanceof tree.Rule) {
        // Check for variable.
        if(typeof(node.variable) === "boolean" && node.variable) {
            return [node];
        }
    }

    // If it has a root ruleset, return all variables from that root.
    if(node.root instanceof tree.Ruleset) {
        return getAllVariables(node.root);
    }

    // If it's a ruleset, map to children.
    if(node instanceof tree.Ruleset) {
        return node.rules.reduce(function(previous, current) {
            return previous.concat(getAllVariables(current));
        }, []);
    }

    // If it's nothing, return empty array.
    return [];
}
